  ...
  <main class="workspace">
    ...
    <aside class="activity">
      ...
      <section class="management">
        <div class="section-heading"><span>授权卡管理</span><b id="license-count">0</b></div>
        <div class="management-form"><input id="license-key" maxlength="256" placeholder="自定义卡密（可选）"><input id="license-label" maxlength="128" placeholder="卡备注（可选）"><input id="license-expiry" type="number" min="0" placeholder="到期 Unix 秒，留空永久"><button id="create-license">生成新卡密</button></div>
        <div id="license-list" class="compact-list"></div>
      </section>
      <section class="management">
        <div class="section-heading"><span>公共选手库审核</span><b id="submission-count">0</b></div>
        <div id="submission-list" class="compact-list"></div>
        <div class="library-summary" id="library-summary"></div>
      </section>
      <section class="management">
        <div class="section-heading"><span>主播 OCR 管控</span></div>
        <div class="management-form"><input id="ocr-device-id" maxlength="128" placeholder="设备 ID"><input id="ocr-until" type="number" min="0" placeholder="禁用截止 Unix 秒"><button id="set-ocr-policy">应用策略</button></div>
        <div class="library-summary">只影响服务器下发的截止时间，客户端离线时仍按已缓存策略执行。</div>
      </section>
    </aside>
  </main>
...
(() => {
  ...
  function renderManagement() {
    ...
    text(byId('license-count'), licenses.length);
    const licenseList = byId('license-list'); licenseList?.replaceChildren();
    for (const item of licenses) {
      const row = document.createElement('div'); row.className = 'compact-item';
      const copy = document.createElement('span'); const title = document.createElement('strong');
      text(title, (item.label || '未备注') + (item.disabledAt ? ' · 已禁用' : ''));
      const meta = document.createElement('small'); text(meta, item.expiresAt ? '到期 ' + formatTime(item.expiresAt) : '永久'); copy.append(title, meta);
      const action = document.createElement('button'); text(action, item.disabledAt ? '启用' : '禁用');
      action.addEventListener('click', () => mutate('/admin/api/licenses/' + item.id + '/disable', 'POST', { disabled: !item.disabledAt }));
      row.append(copy, action); licenseList?.append(row);
    }
    if (licenseList && !licenses.length) { const row = document.createElement('div'); row.className = 'library-summary'; text(row, '暂无授权卡'); licenseList.append(row); }
    const submissions = state.pendingLibrarySubmissions || [];
    text(byId('submission-count'), submissions.length);
    const submissionList = byId('submission-list'); submissionList?.replaceChildren();
    for (const item of submissions) {
      const row = document.createElement('div'); row.className = 'compact-item';
      const copy = document.createElement('span'); const title = document.createElement('strong'); text(title, '投稿 #' + item.id);
      const meta = document.createElement('small'); text(meta, item.deviceId + ' · ' + formatTime(item.createdAt)); copy.append(title, meta);
      const action = document.createElement('button'); text(action, '审核通过'); action.addEventListener('click', () => mutate('/admin/api/player-library/submissions/' + item.id + '/approve', 'POST'));
      row.append(copy, action); submissionList?.append(row);
    }
    if (submissionList && !submissions.length) { const row = document.createElement('div'); row.className = 'library-summary'; text(row, '暂无待审核投稿'); submissionList.append(row); }
    const library = state.playerLibrary || { entities: [], revision: 0 };
    text(byId('library-summary'), '公共库版本 r' + library.revision + ' · ' + library.entities.length + ' 个选手实体');
  }
  function render() { renderDirectory(); renderDetail(); renderActivity(); renderManagement(); }
  ...
  byId('create-license')?.addEventListener('click', async () => { const key = byId('license-key')?.value.trim() || ''; const label = byId('license-label')?.value.trim() || ''; const rawExpiry = byId('license-expiry')?.value.trim() || ''; const expiresAt = rawExpiry ? Number(rawExpiry) : null; try { const result = await request('/admin/api/licenses', { method: 'POST', body: JSON.stringify({ key, label, expiresAt }) }); toast('新卡密（只显示一次）：' + result.key); byId('license-key').value = ''; byId('license-label').value = ''; byId('license-expiry').value = ''; await refresh(); } catch (error) { toast('生成失败：' + error.message); } });
