# Production Server For 5.2.2

**Mandatory server-first rollout: upgrade and verify the production server before distributing the 5.2.2 client (update_v522.zip).** A valid manifest does not mean the server supports the 5.2.2 conflict-reporting and administrator batch-resolution runtime.
...
- Server package: `dnf-cloud-match-server-production-5.2.2.zip`.
...
Never POST a real user's card to `/auth/activate` as a deployment probe: that may bind the card. After all checks pass, verify existing library counts and registered card/broadcaster records in the authenticated administrator UI, restore client ingress, then distribute 5.2.2. Keep the backup.

## Server-First Conflict Verification

Use this operator-controlled order for the production rollout:

1. **backup**: verify the independent application, complete data directory, environment, unit and vault-sidecar backup before downtime.
2. **stop**: block client ingress and database writers; let the reviewed installer stop and verify the service before replacement.
3. **install**: run `sudo bash deploy/install.sh` from the verified 5.2.2 package.
4. **preflight**: run both local and public `preflight.sh` commands shown above before allowing clients to update.
5. **start**: confirm the installer restored the intended enabled/active service policy and that `dnf-cloud-match.service` is active.
6. **health**: verify `/health`, `/api/v2/health`, the unauthenticated 401 boundary and local administrator health on port 18881.
7. **admin verification**: open the authenticated administrator library page through the approved private route, inspect conflict counts, verify exact normalized-name selection and review durable entity redirects.
8. **rollback**: if any check fails, keep traffic blocked and restore the complete matching backup set as documented below.

No real conflict batch is executed automatically by packaging, installation, startup, health checks, preflight, or admin verification. A production batch requires a separately authorized operator action and the exact name-confirmation boundary in the administrator UI.
...
./scripts/package-production-5.2.2.ps1 -ValidateOnly
./scripts/package-production-5.2.2.ps1
```
