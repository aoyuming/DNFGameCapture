...
- worker 只把归一化消息写入最多 128 条的有界入站队列，绝不调用用户回调。宿主 UI 线程调用 `DispatchMessages(maxCount)` 排空消息；派发、`SetMessageCallback`、`Configure`、`Stop` 和析构通过同一个递归派发门串行，统一按派发门再状态锁的顺序加锁，worker 永不获取派发门。`DispatchMessages` 在状态锁内取消息、校验 generation、复制回调、释放容量并唤醒 worker，在状态锁外但仍持有派发门时调用；过期 generation 消息只安全擦除。跨线程生命周期调用在旧回调结束前不能返回，而回调同线程调用 `Stop`、`Configure` 或 `SetMessageCallback` 不会自锁。
...
- `DesiredRoom` 保存 generation；`JoinRoom` 必须在同一互斥区内捕获当前 generation、预留结果槽、写命令和更新目标房间，`Configure` 也在同一互斥区推进 generation 并清空目标房间。自动重加入只允许 `desired.generation == activeConfig.generation`，并用 pending/in-flight generation 防止重复 ACK 请求；`Configure` 和 `Leave` 清空这些状态，Join ACK 结算 in-flight。受保护结果无容量时返回 `noCapacity` 并保留待重入房状态，不断开连接；连接循环在每次最长 200ms 的接收轮询后重试，只有 `networkFailure` 触发重连。
...
Expected: PASS，并确认源码不存在 `.detach()`、worker 回调、UI 同步 `WinHttpReceiveResponse` 或无上限消息队列；覆盖派发回调与跨线程 `Stop`/换回调互斥且同线程可重入、过期 generation 消息丢弃、96 个受保护结果与预留混合填满后拒绝且排空恢复、重入房 `noCapacity` 排空后只重试一次且不重连、旧 Join/Configure generation 竞态、完整事件精确边界、`Command` 移动专有析构和安全擦除静态断言。
