...
$name = 'dnf-cloud-match-server-production-5.2.2'
...
try {
    ...
    foreach ($required in @('server.js', 'v2-api.js', 'library-store.js', 'library-conflict-resolution.js',
        'license-store.js', 'license-vault.js', 'migrations/remove-adventure-identifiers.js')) {
    ...
    $documents = @{ 'README-production-5.2.2.md' = 'README.md'; 'README-game-id-only.md' = 'README-game-id-only.md' }
