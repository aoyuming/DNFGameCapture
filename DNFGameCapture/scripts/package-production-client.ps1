...
if (-not $OutputPath) { $OutputPath = Join-Path $root 'deployment-packages\update_v520.zip' }
...
if ($version -ne '5.2.0.0') { throw "Expected EXE version 5.2.0.0, got $version" }
...
foreach ($name in @('autocomplete-worker.js', 'index.html', 'main.js', 'style.css',
...
Write-Host 'Local package only. Upgrade the production server BEFORE publishing the client update.'
