...
$testSource = Join-Path $PSScriptRoot 'license_lease_policy_test.cpp'
$leaseSource = Join-Path $root 'LicenseLease.cpp'

foreach ($path in @($testSource, $leaseSource)) {
    if (-not (Test-Path -LiteralPath $path)) {
        throw "Missing license lease policy test input: $path"
    }
}
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) `
    ('DNFGameCapture-license-lease-' + [guid]::NewGuid().ToString('N'))
$null = New-Item -ItemType Directory -Path $tempRoot
$exe = Join-Path $tempRoot 'license_lease_policy_test.exe'

try {
    $compile = 'call "{0}" -arch=x64 -host_arch=x64 >nul && "{1}" /nologo /std:c++17 /EHsc /W4 /utf-8 /Y- /DUNICODE /D_UNICODE /I"{2}" /Fe:"{3}" /Fo:"{4}\\" "{5}" "{6}"' -f `
        $devCmd, $compiler, $root, $exe, $tempRoot, $testSource, $leaseSource
    & $env:ComSpec /d /s /c $compile
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $exe)) {
        throw "License lease policy test compilation failed with exit code $LASTEXITCODE."
    }

    & $exe
    if ($LASTEXITCODE -ne 0) {
        throw "License lease policy tests failed with exit code $LASTEXITCODE."
    }
}
finally {
...
Write-Host 'License lease policy test check passed.' -ForegroundColor Green
