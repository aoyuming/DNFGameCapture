...
$name = 'dnf-cloud-match-server-production-5.4.0'
...
try {
    ...
    foreach ($required in @(
        'server.js', 'v2-api.js', 'library-store.js', 'library-conflict-resolution.js',
        'license-store.js', 'license-vault.js', 'broadcaster-attribution.js', 'client-ban.js',
        'ip-region.js', 'migrations/remove-adventure-identifiers.js'
    )) {
        if (-not (Test-Path -LiteralPath (Join-Path $payload "dist/$required") -PathType Leaf)) {
            throw "Missing runtime: $required"
        }
    }
    ...
    $documents = @{
        'README-production-5.4.0.md' = 'README.md'
    ...
    try {
        foreach ($entry in $zip.Entries) {
            ...
            if ($actualHash -ne (Get-FileHash -LiteralPath $original -Algorithm SHA256).Hash) {
    ...
    if (-not $resolved.StartsWith($tempRoot, [StringComparison]::OrdinalIgnoreCase) -or
        (Split-Path -Leaf $resolved) -notmatch '^dnf-production-package-[0-9a-f]{32}$') {
