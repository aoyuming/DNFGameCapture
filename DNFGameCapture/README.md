...
这是一个面向 DNF 擂台/赛事场景的自动人头记分工具。桌面端负责捕获游戏或摄像头画面、识别死亡 X、调用 OCR、匹配击杀双方并写出 OBS 可读取的 TXT；WebView2 前端负责给用户操作红蓝队、选手、游戏ID库、云端同步和最近识别复盘。

## 项目结构

- `DNFGameCaptureDlg.cpp/.h`：主窗口和核心业务。包含授权校验、画面捕获调度、死亡 X 检测、OCR 调用、击杀匹配、战绩统计、配置读写、WebView2 消息处理、游戏ID库本地/云端同步。
- `WebScoreDlg.cpp/.h`：WebView2 记分窗口。加载 `web前端/index.html`，负责 C++ 与前端的双向消息桥接。
- `web前端/`：用户主要操作界面。包含红蓝队 4v4 记分、游戏ID绑定/编辑、云端库同步/推送、最近识别、C++ 日志面板等。
- `WGCCapture.cpp/.h`：Windows Graphics Capture 捕获引擎，优先用于低闪烁、高性能窗口捕获。
- `CameraCapture.cpp/.h`：摄像头采集模式，用于外部采集卡/摄像头画面。
- `NameMatcher.hpp`：OCR 文本与游戏ID/角色名的模糊匹配逻辑，包含常见 OCR 错字处理。
- `TemporalIdentityMatcher.hpp`：固定红框身份融合匹配。把 ID、大区、职业等多帧 OCR 证据合并，降低短 ID、符号 ID 和职业帧误判。
- `DNFGameCaptureDlg_IdentityPatch.cpp`：身份融合相关辅助实现，接入主窗口的玩家与游戏ID数据。
- `云函数/`：Node/Express 云端服务，使用阿里云 OSS 做 CDK 授权、公共游戏ID库、投稿审核区和管理员直写。
- `秘钥后台管理/`：共享游戏ID库后台管理工具，包含 CLI、网页后台和打包说明。
- `秘钥生成器/`：CDK 生成器。
- `res/`：MFC 图标和资源文件。

## 核心流程

1. 用户在 WebView2 前端填写红队/蓝队选手和游戏ID。
2. C++ 主程序通过 WebView2 消息接收前端状态，并同步到 `m_players` 和本地游戏ID库。
3. 开始监控前会检查授权、Umi-OCR 服务、游戏ID完整性和死亡 X 算法配置。
...
7. `NameMatcher` 和 `TemporalIdentityMatcher` 综合 OCR 文本、游戏ID元信息、队伍位置和时间窗缓存，确认击杀者/死亡者。
...
- `alias_cloud_baseline.json`：云端共享游戏ID库同步基线，用于判断本地库是否有新增/删除变化。
...
- `cmd_auth`：输入 CDK 授权。
- `cmd_sync_alias_db`：从云端公共库同步游戏ID。
- `cmd_push_alias_db`：提交本地游戏ID库到云端待审核。
- `cmd_direct_sync_alias_db`：管理员直写公共游戏ID库。
- `cmd_delete_alias`：永久解绑游戏ID。
- `cmd_reset_stats`：重置战绩。

## 游戏ID库规则

- 选手只用于展示和归属；OCR 匹配主要使用游戏ID。
...
- `shared-alias/public_alias_db.json`：公共游戏ID库。
- `shared-alias/admin_key_hashes.json`：管理员直写白名单。
- `alias-submissions/pending/`：用户提交的待审核游戏ID库变更。
...
- Web 前端状态不是单独权威源，C++ 会回推完整状态；修改游戏ID、选手和战绩时要同时考虑 C++ 与 Web 的同步方向。
